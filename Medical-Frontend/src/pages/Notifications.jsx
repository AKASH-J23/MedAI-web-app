      ...
      <Tabs className='border p-2 border-black'>
        ...
        <hr className='border-black'/>
          <div className="flex justify-end">
            <h1 className="cursor-pointer hover:text-red-500 mt-2" onClick={()=>markAllAsSeen()}>Mark all as seen</h1>
          </div>
          {/* <hr className='border-black'/> */}

          {user?.unseenNotifications.map((notification,index) => (
            <div key={`unseen-${index}`} className=" p-1 mt-1" onClick={()=>navigate(notification.onClickPath)}>
        ...
        <hr className='border-black'/>
          <div className="flex justify-end">
            <h1 className="cursor-pointer hover:text-red-500 mt-2" onClick={()=>deleteAll()}>Delete all</h1>
          </div>
          {user?.seenNotifications.map((notification, index) => (
            <div key={`seen-${index}`} className="p-1 mt-1" onClick={()=>navigate(notification.onClickPath)}>
                <div className="bg-white rounded p-2">{notification.message}</div>
            </div>
          ))}
        </Tabs.TabPane>
